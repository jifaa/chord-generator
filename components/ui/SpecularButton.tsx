...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    // Graceful teardown: cancel animation, remove canvas, release GPU resources
    const teardown = () => {
      cancelAnimationFrame(raf);
      ro.disconnect();
      window.removeEventListener('pointermove', onPointerMove);
      mm?.removeEventListener('change', mmListener);
      if (gl.canvas.parentNode === fx) fx.removeChild(gl.canvas);
      gl.getExtension('WEBGL_lose_context')?.loseContext();
    };

    const mmListener = (e: MediaQueryListEvent) => {
      if (e.matches) teardown(); // Gracefully stop WebGL when reduced-motion activates
    };
    mm?.addEventListener('change', mmListener);
  ...
  return (
    ...
    >
      <span ref={fxRef} aria-hidden="true" data-specular-fx className="pointer-events-none absolute -inset-5 z-[1] [&_canvas]:block [&_canvas]:h-full [&_canvas]:w-full" />
