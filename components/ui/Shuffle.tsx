    ...
    () => {
      ...
      const randomizeScramble = () => {
        ...
        wrappersRef.current.forEach(w => {
          ...
          for (let i = 1; i < kids.length - 1; i++) {
            (kids[i] as HTMLElement).textContent = scrambleCharset.charAt(Math.floor(Math.random() * scrambleCharset.length));
          }
        });
      };
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            if (scrambleCharset) randomizeScramble();
            if (isVertical) {
              gsap.set(strips, { y: (i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-start-y') || '0') });
            } else {
              gsap.set(strips, { x: (i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-start-x') || '0') });
            }
            onShuffleComplete?.();
          },
          onComplete: () => {
        ...
        const addTween = (targets: Element[], at: number) => {
          ...
          if (isVertical) {
            vars.y = (i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-final-y') || '0');
          } else {
            vars.x = (i: number, t: Element) => parseFloat((t as HTMLElement).getAttribute('data-final-x') || '0');
          }
        ...
        if (animationMode === 'evenodd') {
          ...
          strips.forEach(strip => {
            ...
            if (isVertical) {
              vars.y = parseFloat((strip as HTMLElement).getAttribute('data-final-y') || '0');
            } else {
              vars.x = parseFloat((strip as HTMLElement).getAttribute('data-final-x') || '0');
            }
            tl.to(strip, vars, d);
